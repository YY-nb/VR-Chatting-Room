...
namespace BNG {
    ...
    public class HandPoser : MonoBehaviour {
        ...
        public virtual void UpdateHandPose(HandPoseDefinition pose, bool lerp) {
            UpdateJoint(pose.WristJoint, WristJoint, lerp, UpdateWristPosition, UpdateJointRotations); //print(pose.WristJoint.TransformName + " ,!" + WristJoint.name);
        ...
        public virtual void UpdateJoint(FingerJoint fromJoint, Transform toTransform, float lerpAmount, bool updatePosition, bool updateRotation)
        {
            ...
            if (lerpAmount != 1)
            {
                ...
                {
                    toTransform.localRotation = Quaternion.Lerp(toTransform.localRotation, fromJoint.LocalRotation, lerpAmount); //print(toTransform.localRotation.eulerAngles);
                }
            }
            ...
            else
            {
                ...
                if (UpdateJointRotations)
                {
                    toTransform.localRotation = fromJoint.LocalRotation; //print($"amount=1,{toTransform.name}: {toTransform.localRotation.eulerAngles}");
        ...
        public virtual void UpdateJoints(List<FingerJoint> joints, List<Transform> toTransforms, float lerpAmount) {
            ...
            for (int x = 0; x < jointCount; x++) {
                ...
                if (x < toTransforms.Count) {
                    ...
                    //print($"{joints[x].TransformName}, to :{toTransforms[x].name},verify:{verifyTransformName}");
                    if (verifyTransformName && joints[x].TransformName == toTransforms[x].name) {
                       // print($"{joints[x].TransformName}, to :{toTransforms[x].name},amount:{lerpAmount}");
                        ...
                        //print($"{joints[x].TransformName}, to :{toTransforms[x].name},falseVerify,amount:{lerpAmount}");
                        UpdateJoint(joints[x], toTransforms[x], lerpAmount, UpdateJointPositions, UpdateJointRotations);
                    }
                }
            }
