...
public class NetworkedMarker : GrabbableEvents, IPunObservable
{
    ...
    [PunRPC]
    void isRemoteDrawing(bool draw)
    {
        if (draw)
        {
            if (drawRoutine == null)
    ...
    public override void OnGrab(Grabber grabber)
    {
        ...
        {
            isDrawing = true;
            view.RPC("isRemoteDrawing", RpcTarget.All, true);
            drawRoutine = StartCoroutine(WriteRoutine());
        }
    ...
    public override void OnRelease()
    {
        if (drawRoutine != null)
        {
            ...
            drawRoutine = null;
            isDrawing = false;
            view.RPC("isRemoteDrawing", RpcTarget.All, false);
        }
    ...
    IEnumerator WriteRoutine()
    {
        while (true)
        {
            if (Physics.Raycast(RaycastStart.position, RaycastStart.up, out RaycastHit hit, RaycastLength,
                    DrawingLayers, QueryTriggerInteraction.Ignore))
            {
    ...
    //It is still here don't know why
