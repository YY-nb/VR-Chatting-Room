...
public class NetworkedHandJet : GrabbableEvents, IPunObservable
{
    ...
    void DoJet(float triggerValue)
    {
        ...
        {
            JetFX.Play();
            photonView.RPC(nameof(ControlJetFX), RpcTarget.Others, true);
        }
    ...
    [PunRPC]
    private void ControlJetFX(bool needPlay)
    {
        if (JetFX != null)
        {
            if (needPlay)
    ...
    void StopJet()
    {
        ...
        {
            JetFX.Stop();
            photonView.RPC(nameof(ControlJetFX), RpcTarget.Others, false);
        }
